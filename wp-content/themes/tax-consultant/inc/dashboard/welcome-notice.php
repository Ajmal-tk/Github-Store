 ...
 */
if (!function_exists('tax_consultant_is_plugin_installed')) {
    function tax_consultant_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('tax_consultant_is_plugin_activated')) {
    function tax_consultant_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_tax_consultant_install_and_activate_plugins', 'tax_consultant_install_and_activate_plugins');
add_action('wp_ajax_nopriv_tax_consultant_install_and_activate_plugins', 'tax_consultant_install_and_activate_plugins');
add_action('wp_ajax_tax_consultant_rplugin_activation', 'tax_consultant_rplugin_activation');
add_action('wp_ajax_nopriv_tax_consultant_rplugin_activation', 'tax_consultant_rplugin_activation');
...
function tax_consultant_all_plugins_active() {
...
function tax_consultant_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('tax_consultant_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_tax_consultant_plugin_installed($plugin_slug)) {
...
function is_tax_consultant_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_tax_consultant_dismissed_notice_handler', 'tax_consultant_ajax_notice_dismiss_function');

function tax_consultant_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'tax_consultant_welcome_nonce')) {
...
function tax_consultant_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        $tax_consultant_current_screen = get_current_screen();
        $tax_consultant_theme = wp_get_theme();
        if ($tax_consultant_current_screen && $tax_consultant_current_screen->id !== 'appearance_page_tax-consultant-guide-page') {
            ?>

            <div class="tax-consultant-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="tax-consultant-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($tax_consultant_theme->get('Name')); ?></p>
                        <p><?php _e('For information and detailed instructions, check out our theme documentation.', 'tax-consultant'); ?></p>
                    </div>
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" id="install-activate-button" href="#"><?php _e('Begin Installation', 'tax-consultant'); ?></a>`
                    </div>
                </div>
            </div>
            <?php
...
add_action('admin_notices', 'tax_consultant_custom_admin_notice');

// After switching theme, reset dismissed notice option
add_action('after_switch_theme', 'tax_consultant_after_switch_theme');
function tax_consultant_after_switch_theme() {
